...
字符串可以使用某些占位符，随后可以调用 `format` 方法，将这些占位符替换为 `format` 方法的相应参数。

观察第一个用法，我们使用 `{0}`，它对应变量 `name`，这是 format 方法的第一个参数。同样，第二个占位符是 `{1}`，对应 `age`，这是 format 方法的第二个参数。注意 Python 从 0 开始计数，这意味着第一个位置在索引 0 处，第二个位置在索引 1 处，依此类推。
...
Python 在 `format` 方法中所做的是将每个参数值替换到占位符的位置。还可以有更详细的格式规范，例如：
